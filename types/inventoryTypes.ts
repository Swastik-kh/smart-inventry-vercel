...
export interface LeaveApplication {
  id: string;
  fiscalYear: string;
  date: string; // Application date (BS)
  applicant: {
    id: string;
    name: string;
    designation: string;
  ...
  leaveDetails: {
    type: 'Bhaipari' | 'Ghar' | 'Birami' | 'Prasuti' | 'Kiriya' | 'Adhyayan' | 'Asadharan' | 'Other';
    otherType?: string;
    startDate: string;
    endDate: string;
  ...
  adminSection?: {
    ...
    currRequest: string;
    remaining: string;
    checkedBy?: string; // Name of admin staff
  };
  recommendation?: {
    recommenderId?: string; // User ID selected by applicant
    recommenderName?: string;
    status: 'Pending' | 'Recommended' | 'Not Recommended';
    date?: string;
    remarks?: string;
  };
  approval?: {
    ...
    returnDate?: string; // Date to return to office
  };
  status: 'Pending Recommendation' | 'Pending Approval' | 'Approved' | 'Rejected';
}
